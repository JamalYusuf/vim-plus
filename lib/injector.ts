...
var VimPlusInjector: VimPlusInjectorTy | undefined | null
((): void => {
  const old = VimPlusInjector, cur: VimPlusInjectorTy = {
  ...
  if (old) {
    for (let key of <Array<keyof VimPlusInjectorTy>> Object.keys(old)) {
      (cur as Generalized<VimPlusInjectorTy>)[key] = old[key]
    }
  }
  cur.alive = -1
  cur.$g = null
...
(function (_a0: 1, injectorBuilder: (scriptSrc: string) => VimPlusInjectorTy["reload"]): void {
...
function handler(this: void, res: ExternalMsgs[kFgReq.inject]["res"] | undefined | false, err?: LastError): void {
  ...
  if (!res) {
    ...
    if (!noBackend) {
      ...
      console.log("%cVim+%c: %cfail%c to inject into %c%s%c %s"
  ...
  const newInjector = VimPlusInjector = {
    ...
    $m (task): void { VimPlusInjector && VimPlusInjector.$r(typeof task === "object" ? task.t : task); },
    $r (): void { /* empty */ },
...
if (document.readyState !== "loading") {
...
})(1, function (scriptSrc): VimPlusInjectorTy["reload"] {
  return function (isAsync): void {
    ...
    function doReload(): void {
      ...
      console.log("%cVim+%c begins to reload%s."
...
(!document.currentScript
  ...
  ) && VimPlusInjector.clickable !== null &&
(function (): void {
...
VimPlusInjector.clickable = VimPlusInjector.clickable
