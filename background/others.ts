import {
  ...
  framesForTab_, onInit_, updateHooks_, settingsCache_, copy_
} from "./store"
import {
  Tabs_, browser_, getCurWnd, runtimeError_, watchPermissions_, browserWebNav_, runContentScriptsOn_, import2, Q_
} from "./browser"
...
void settings_.ready_.then((): void => {
  ...
  if (settingsCache_.showContextMenu !== false) {
    setupContextMenus_(true)
  }
  setupAlarms_()
...
type CtxMenusApi = {
  ...
  create (createProperties: Dict<any>, cb?: () => void): void
  onClicked: { addListener (cb: (info: Dict<any>, tab?: Tab) => void): void }
}

const setupContextMenus_ = (enable: boolean): void => {
  const menus = (browser_ as Dict<any>)["contextMenus"] as CtxMenusApi | undefined
  if (!menus) { return }
  menus.removeAll((): void => {
    runtimeError_()
    if (!enable) { return }
    const mk = (id: string, title: string, contexts: string[]): void => {
      menus.create({ id, title, contexts }, runtimeError_)
    }
...
updateHooks_.showContextMenu = (value): void => { setupContextMenus_(!!value) }

{
  const menus = (browser_ as Dict<any>)["contextMenus"] as CtxMenusApi | undefined
  menus && menus.onClicked.addListener((info, tab): void => {
    ...
    case MENU_IDS.searchSelection: {
      const text = ((info["selectionText"] as string) || "").trim()
      if (!text) { break }
      openUrlReq({ u: text, r: ReuseType.newFg })
      break
    }
    case MENU_IDS.copyLink: {
      const url = info["linkUrl"] as string | undefined
      if (!url) { break }
      void Promise.resolve(copy_(url)).then(blank_, blank_)
      break
    }
    case MENU_IDS.excludeSite: {
      if (!tab || !tab.url) { break }
      const url = tab.url
      const m = (<RegExpOne> /^(https?:\/\/[^/]+)/).exec(url)
      if (!m) { break }
      const pattern = `:${m[1]}/`
    ...
    case MENU_IDS.readingList: {
      ...
      const url = (info["linkUrl"] as string) || tab.url || ""
      if (!rl || !(<RegExpOne> /^https?:/).test(url)) { break }
      void rl.addEntry({ title: (tab.title || url).slice(0, 255), url, hasBeenRead: false }).then(blank_, blank_)
      break
    }
    case MENU_IDS.bookmark: {
      if (!tab || !tab.url || !browser_.bookmarks) { break }
      void Q_(browser_.bookmarks.search, { url: tab.url }).then((found): void => {
        if (found && found.length) {
          void Q_(browser_.bookmarks!.remove, found[0].id).then(blank_, blank_)
        } else {
          void Q_(browser_.bookmarks!.create, { title: tab!.title || tab!.url, url: tab!.url }).then(blank_, blank_)
        }
      }, blank_)
      break
    }
...
const setupAlarms_ = (): void => {
  const alarms = (browser_ as Dict<any>)["alarms"] as {
    create (name: string, info: { periodInMinutes: number }): void
    onAlarm: { addListener (cb: (alarm: { name: string }) => void): void }
  } | undefined
  if (!alarms) { return }
  alarms.create("vim-plus-tick", { periodInMinutes: 1 })
  alarms.onAlarm.addListener((alarm): void => {
    if (alarm.name !== "vim-plus-tick") { return }
    try {
      const session = browser_.storage && (browser_.storage as Dict<any>)["session"] as {
        get (k: string): Promise<unknown>
      } | undefined
      session && void session.get("recency").then(blank_, blank_)
    } catch { /* empty */ }
  })
}
...
installation_ && void Promise.all([installation_, settings_.ready_]).then(([details]): void => {
  ...
  console.log("%cVim+%c has been %cinstalled%c with %o at %c%s%c.", "color:red", "color:auto"
      , "color:#0c85e9", "color:auto", details, "color:#0c85e9", BgUtils_.now(), "color:auto")

  if (CONST_.DisallowIncognito_) {
    console.log("Sorry, but some commands of Vim+ require the permission to run in incognito mode.");
  }
