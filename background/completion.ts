...
import { browser_, getGroupId, getTabUrl, isTabMuted, Windows_, runtimeError_, import2 } from "./browser"
...
windowEngine = {
  filter_ (query: CompletersNS.QueryStatus, _index: number): void {
    ...
    Windows_.getAll({ populate: true }, (wnds): void => {
      if (query.o || runtimeError_()) {
        Completers.next_([], SugType.tab)
        return runtimeError_()
      }
      const sugs: CompletersNS.TabSuggestion[] = []
      ...
      for (const wnd of list) {
        ...
        if (queryTerms.length && !match2_(text, title + " " + url)) { continue }
        n++
        const label = wnd.id === curWndId_ ? `(W${n})` : `W${n}`
        const sug = new Suggestion("tab", url, text, title, get2ndArg,
            wnd.focused || wnd.id === curWndId_ ? 1e9 : 1e6 - n) as CompletersNS.TabSuggestion
        sug.s = active.id
        sug.label = `#${label}${wnd.incognito ? " *" : ""}${wnd.state === "minimized" ? " ~" : ""}`
        sug.visit = recencyForTab_.get(active.id) || 0
        sugs.push(sug)
      }
      matchedTotal += sugs.length
      if (offset > 0 && sugs.length > offset) {
        Completers.next_(sugs.slice(offset, offset + maxResults), SugType.tab)
      } else {
        Completers.next_(sugs.slice(0, maxResults + offset), SugType.tab)
      }
      UrlDecoder_.continueToWork_()
    })
  }
  ...
  preFilter_: function (query: CompletersNS.QueryStatus, failIfNull?: true | null
    ...
    if (failIfNull) {
      ...
      if (q.length === 1 && keyword && !offset) {
        const keys = SearchKeywords_.matchingKeys_(keyword, Math.min(maxResults, 8))
        if (keys.length) {
          const sugs: SearchSuggestion[] = []
          for (const key of keys) {
            const eng = searchEngines_.map.get(key)
            if (!eng) { continue }
            const blank = eng.blank_ || eng.url_.split("$")[0]
            const sug = new Suggestion("search", blank || eng.url_, key + " ",
                eng.name_ + "  ·  " + key, get2ndArg, 9.5 - sugs.length * 0.05) as SearchSuggestion
            sug.t = key + " "
            sug.textSplit = OnFirefox && isForAddressBar ? sug.t : BgUtils_.escapeText_(sug.t)
            sug.title = (OnFirefox && isForAddressBar)
                ? eng.name_
                : cutTitle(eng.name_ + "  ·  " + key, [0, eng.name_.length])
            sug.p = isForAddressBar ? key : ""
            sug.v = OnChrome && !isForAddressBar
                ? (blank && blank.startsWith("http") ? blank : calcBestFaviconSource_only_cr_!(blank || eng.url_))
                : ""
            sugs.push(sug)
          }
          if (sugs.length) {
            matchType = MatchType.searching_
            return Completers.next_(sugs, SugType.search)
          }
        }
      }
      return Completers.next_([], SugType.search);
...
knownCs = {
  ...
  window: [SugType.tab as never, windowEngine] as CompleterList
}

const import2QuickActions_ = (): Promise<typeof import("./quick_actions")> =>
    import2<typeof import("./quick_actions")>("/background/quick_actions.js")
...
Completion_.filter_ = (query: string, options: CompletersNS.FullOptions, callback: CompletersNS.Callback): void => {
    ...
    if (query.startsWith(":") && !(<RegExpOne> /^:[bBhHtTwWsSoOd](\s|$)/).test(query)) {
      void import2QuickActions_().then((mod): void => {
        ...
        const sugs = rows.map((row, i): CompletersNS.Suggestion => {
          // text (t) must stay the short command so the bar stays clean when selecting
          const cat = (row as { cat?: string }).cat || "Action"
          const iconType = typeof (mod as any).qaIconType_ === "function"
              ? (mod as any).qaIconType_(cat) as string
              : "search"
          const sug = new Suggestion("search", row.url, row.text, row.title, get2ndArg, 9 - i * 0.01
              ) as CompletersNS.SearchSuggestion
          sug.textSplit = BgUtils_.escapeText_(row.text + "  ·  " + row.desc)
          sug.title = cutTitle(row.title, [0, Math.min(row.title.length, 56)])
          sug.label = cat
          sug.p = ":"
          sug.n = 1
          // Category icon class (qa-read, qa-view, …) — see vomnibar.html path colors
          ;(sug as any).e = iconType
          return sug
        })
        if (!sugs.length) {
          const sug = new Suggestion("search", "vimium://qa/a", ":a", "No match", get2ndArg, 1
              ) as CompletersNS.SearchSuggestion
          sug.t = ":a"
          sug.textSplit = "Try :view :tab :hist :priv :win :nav :clip · or :gray :h1 :pin"
          sug.label = "Help"
          sug.p = ":"
          sugs.push(sug)
        }
        callback(sugs, true, MatchType.someMatches, SugType.search, sugs.length, "action", CompletersNS.QComponent.query)
      })
      return
    }
