...
export const SearchKeywords_ = {
  ...
  matchingKeys_ (prefix: string, limit: number): string[] {
    if (!prefix || limit < 1) { return [] }
    const arr = searchEngines_.keywords
    if (arr === null) {
      SearchKeywords_._timer = SearchKeywords_._timer || setTimeout(SearchKeywords_._buildSearchKeywords, 67)
      SearchKeywords_._buildSearchKeywords()
    }
    const keys = (searchEngines_.keywords || "").split("\n")
    const out: string[] = []
