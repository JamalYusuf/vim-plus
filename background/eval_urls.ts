...
set_evalVimiumUrl_(function (path: string, workType?: Urls.WorkType
  ...
  case "qa": case "quick": case "action": case "actions": {
    // Omnibar ":" quick actions — privacy / history helpers
    return import2<typeof import("./quick_actions")>("/background/quick_actions.js")
        .then((mod): Promise<Urls.BaseEvalResult> => {
          const r = mod.runQuickAction_(path)
          return r instanceof Promise ? r as Promise<Urls.BaseEvalResult> : Promise.resolve(r as Urls.BaseEvalResult)
        })
  }
