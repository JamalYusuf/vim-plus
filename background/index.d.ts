...
declare namespace SettingsNS {
  interface BackendSettings extends BaseBackendSettings {
    ...
    hashbangs: string;
    /**
     * Percent of the *previous* dock size kept on each repeat of the same direction (25–90).
     * First press always uses half the work area; second ≈ half×(step/100), etc.
     */
    dockWindowStep: number;
    /** Top reading-progress bar on pages (content script). Default on. */
    readingProgress: boolean
    readingProgressColor: string
    readingProgressHeight: number
    /**
     * Extra CSS for the progress bar (appended after built-in track/fill rules).
     * Targets #vp-read-progress-track and #vp-read-progress-fill.
     */
