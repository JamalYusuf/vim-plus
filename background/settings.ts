...
Object.assign<typeof updateHooks_, { [key in SettingsNS.DeclaredUpdateHooks]: SettingsNS.UpdateHook<key> }>(
    ...
    searchEngines (): void {
      ...
      searchEngines_.rules = parseSearchEngines_("~:" + settingsCache_.searchUrl
          ...
          + normalizeHashbangsForParser_(settingsCache_.hashbangs || "") + "\n"
...
updateHooks_.hashbangs = (_value: string): void => {
  updateHooks_.searchEngines!(settingsCache_.searchEngines, "searchEngines")
}
 ...
 */
export const normalizeHashbangsForParser_ = (raw: string): string => {
  if (!raw) { return "" }
  const hasUrlLike = <RegExpI> /https?:|vimium:|javascript:|\$s|%s|\$S/i
  return raw.split("\n").map((line): string => {
    const trimmed = line.trim()
    if (!trimmed || trimmed[0] !== "!") { return line }
    // Real comments: "! something" without a URL rule
    let colon = 0
    do { colon = trimmed.indexOf(":", colon + 1) } while (colon > 0 && trimmed.charCodeAt(colon - 1) === 92 /* \\ */)
    if (colon <= 0) { return line }
    const rest = trimmed.slice(colon)
    // Keep pure comments like "! note" without a URL
    if (!hasUrlLike.test(rest)) { return line }
    ...
    if (nonBang) {
      ...
      ordered = plain
          ? [plain, first, ...keys.slice(1).filter(k => k !== plain && k !== first)]
          : keys
    }
    // Preserve original indentation
    const lead = line.match(<RegExpOne> /^\s*/)
...
,\u4e0b\u4e00\u5f20,next,more,newer,>,\u203a,\u2192,\xbb,\u226b,>>",
    ...
    passEsc: GlobalConsts.kCssDefault as const
        || "[aria-controls],[role=combobox],#kw.s_ipt" // Bing / Google search boxes
