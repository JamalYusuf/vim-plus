...
const parseHTML = (template: string): [string, string] => {
    ...
    const wiki = browser_.runtime.getURL("pages/wiki.html")
    const options = browser_.runtime.getURL("pages/options.html")
...
const commandGroups_: {
  ...
  tabManipulation: [
    ...
    , "toggleTabGroup", 1, "$title, color"
    , "collapseTabGroup", 1, "$collapsed"
    , "renameTabGroup", 1, "$title, color"
    , "moveTabToGroup", 1, "$title"
    , "openSidePanel", 1
    , "addToReadingList", 1
    , "toggleBookmark", 1
    , "cycleWindows", 1
    , "dockWindow", 1, "$direction=left|right|up|down|max|center, step, first"
    , "dockWindowLeft", 1
    , "dockWindowRight", 1
    , "dockWindowUp", 1
    , "dockWindowDown", 1
    , "dockWindowMax", 1
    , "showLastDownload", 1, "$open"
    , "openDownloads", 1
    , "openHistoryPage", 1
    , "openExtensions", 1
    , "openShortcuts", 1
    , "enterPictureInPicture", 1
    , "togglePinTab"
