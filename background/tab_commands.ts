import {
  ...
  Origin2_, lastWndId_, settingsCache_
} from "./store"
...
export const moveTabToNewWindow = (resolve: OnCmdResolved): void | kBgCmd.moveTabToNewWindow => {
  ...
  const moveTabToNewWindow0 = (wnd: PopWindow): void => {
    ...
    const oldGroupByTab = new Map<number, number>()
    if (!OnEdge && !OnFirefox) {
      for (const i of tabs) {
        const g = getGroupId(i)
        if (g != null && typeof g === "number") { oldGroupByTab.set(i.id, g) }
      }
    }
    if (!all) {
    ...
    makeWindow(wndInit, wndState, (wnd2?: Window): void => {
      ...
      if (oldGroupByTab.size && browser_.tabGroups) {
        ...
        byOldGroup.forEach((tabIds): void => {
          if (tabIds.length) {
            tasks.push(Tabs_.group({ tabIds, createProperties: { windowId: wnd2!.id } }))
          }
        })
        void Promise.all(tasks).then((): void => { resolve(1) }, (): void => { resolve(1) })
        return
      }
      resolve(1)
    })
...
const noTabGroups = (): boolean => !browser_.tabGroups || OnEdge || OnFirefox

export const openSidePanel = (tabs: [Tab], resolve: OnCmdResolved): void | kBgCmd.openSidePanel => {
  // Prefer the dedicated kFgReq.key fast-path. This fallback still tries, then explains.
  void import("./side_panel.js" as string).then((m: typeof import("./side_panel")): void => {
    const tab = tabs && tabs[0]
    const started = m.openSidePanelImmediate_(tab ? tab.id : undefined, tab ? tab.windowId : undefined)
    void m.openSidePanelBestEffort_(tab || null).then((ok): void => {
      if (!ok && !started) { m.explainSidePanelGesture_() }
      resolve(ok || started ? 1 : 0)
    })
  }, (): void => {
    showHUD("Side panel module failed to load")
    resolve(0)
  })
}

export const toggleTabGroup = (tabs: [Tab], resolve: OnCmdResolved): void | kBgCmd.toggleTabGroup => {
  if (noTabGroups()) { showHUD("Tab groups unavailable"); resolve(0); return }
  const tab = tabs[0]
  const groupId = getGroupId(tab)
  if (groupId != null) {
    Tabs_.ungroup([tab.id], (): void => {
      showHUD("Ungrouped tab")
      resolve(1)
      return runtimeError_()
    })
    return
  }
  const title = (get_cOptions<C.toggleTabGroup>().title || "") + ""
  const color = get_cOptions<C.toggleTabGroup>().color as chrome.tabGroups.TabGroup["color"] | undefined
  void Tabs_.group({ tabIds: [tab.id] }).then((newGroupId): void => {
    if (newGroupId == null) { resolve(0); return }
    if (title || color) {
      void browser_.tabGroups!.update(newGroupId, {
        title: title || undefined,
        color: color || undefined
      })
    }
    showHUD(title ? `Grouped: ${title}` : "Tab grouped")
    resolve(1)
  }, (): void => { resolve(0) })
}

export const collapseTabGroup = (tabs: [Tab], resolve: OnCmdResolved): void | kBgCmd.collapseTabGroup => {
  if (noTabGroups()) { showHUD("Tab groups unavailable"); resolve(0); return }
  const groupId = getGroupId(tabs[0])
  if (groupId == null || typeof groupId !== "number") {
    showHUD("Active tab is not in a group")
    resolve(0)
    return
  }
  void browser_.tabGroups!.query({ windowId: tabs[0].windowId }).then((raw): void => {
    const groups = (raw as unknown as chrome.tabGroups.TabGroup[] | undefined) || []
    const info = groups.find(g => g.id === groupId)
    if (!info) { resolve(0); return }
    const force = get_cOptions<C.collapseTabGroup>().collapsed
    const collapsed = force != null ? !!force : !info.collapsed
    void browser_.tabGroups!.update(groupId, { collapsed }).then((): void => {
      showHUD(collapsed ? "Group collapsed" : "Group expanded")
      resolve(1)
    }, (): void => { resolve(0) })
  }, (): void => { resolve(0) })
}

export const renameTabGroup = (tabs: [Tab], resolve: OnCmdResolved): void | kBgCmd.renameTabGroup => {
  if (noTabGroups()) { showHUD("Tab groups unavailable"); resolve(0); return }
  const groupId = getGroupId(tabs[0])
  if (groupId == null || typeof groupId !== "number") {
    showHUD("Active tab is not in a group")
  ...
  let title = (get_cOptions<C.renameTabGroup>().title || "") + ""
  const color = get_cOptions<C.renameTabGroup>().color as chrome.tabGroups.TabGroup["color"] | undefined
  if (!title) {
    const url = getTabUrl(tabs[0])
    const host = (<RegExpOne> /^https?:\/\/([^/]+)/).exec(url)
    title = host ? host[1].replace(<RegExpOne> /^www\./, "") : "Group"
  }
  void browser_.tabGroups!.update(groupId, {
...
export const moveTabToGroup = (tabs: [Tab], resolve: OnCmdResolved): void | kBgCmd.moveTabToGroup => {
  if (noTabGroups()) { showHUD("Tab groups unavailable"); resolve(0); return }
  const tab = tabs[0]
  const wantedTitle = (get_cOptions<C.moveTabToGroup>().title || "") + ""
  void browser_.tabGroups!.query({ windowId: tab.windowId }).then((raw): void => {
    const groups = (raw as unknown as chrome.tabGroups.TabGroup[] | undefined) || []
    if (!groups.length) {
      showHUD("No tab groups in this window")
      resolve(0)
      return
    }
    let target = wantedTitle
        ? groups.find(g => (g.title || "").toLowerCase() === wantedTitle.toLowerCase())
        : null
    if (!target) {
      const curG = getGroupId(tab)
      let idx = groups.findIndex(g => g.id === curG)
      if (idx < 0) { idx = -1 }
      const next = (idx + (cRepeat >= 0 ? 1 : -1) + groups.length * 10) % groups.length
      target = groups[next]
    }
    if (!target) { resolve(0); return }
    void Tabs_.group({ groupId: target.id, tabIds: [tab.id] }).then((): void => {
      showHUD(`Moved to group: ${target!.title || target!.id}`)
      resolve(1)
    }, (): void => { resolve(0) })
  }, (): void => { resolve(0) })
}

export const addToReadingList = (tabs: [Tab], resolve: OnCmdResolved): void | kBgCmd.addToReadingList => {
  const rl = (browser_ as Dict<any>)["readingList"] as {
    addEntry (entry: { title: string, url: string, hasBeenRead?: boolean }): Promise<void>
  } | undefined
  if (!rl || !rl.addEntry) {
    showHUD("Reading List API unavailable")
    resolve(0)
    return
  }
  const tab = tabs[0]
  const url = getTabUrl(tab)
  if (!url || !(<RegExpOne> /^https?:/).test(url)) {
    showHUD("Only http(s) pages can be added to Reading List")
    resolve(0)
    return
  }
  const title = ((get_cOptions<C.addToReadingList>().title || tab.title || url) + "").slice(0, 255)
  void rl.addEntry({ title, url, hasBeenRead: false }).then((): void => {
    showHUD("Added to Reading List")
    resolve(1)
  }, (err: unknown): void => {
    const msg = err && (err as { message?: string }).message || "failed"
    showHUD("Reading List: " + msg)
    resolve(0)
  })
}

export const cycleWindows = (resolve: OnCmdResolved): void | kBgCmd.cycleWindows => {
  const includeMin = get_cOptions<C.cycleWindows>().minimized !== false
  Windows_.getAll((wnds): void => {
    if (!wnds || !wnds.length) { resolve(0); return runtimeError_() }
    let list = wnds.filter(w => w.type === "normal" || w.type === "popup")
    if (!includeMin) {
      list = list.filter(w => w.state !== "minimized")
    }
    if (list.length < 2) {
      showHUD(list.length ? "Only one window" : "No windows")
      resolve(0)
      return
    }
    list.sort((a, b) => a.id - b.id)
    const curId = curWndId_
    let idx = list.findIndex(w => w.id === curId)
    if (idx < 0) { idx = 0 }
    const delta = cRepeat >= 0 ? 1 : -1
    const steps = Math.max(1, Math.abs(cRepeat | 0) || 1)
    const next = list[(idx + delta * steps + list.length * 100) % list.length]
    Windows_.update(next.id, { focused: true }, (): void => {
      if (next.state === "minimized") {
        Windows_.update(next.id, { state: "normal" }, runtimeError_)
      }
      showHUD("Window " + (list.indexOf(next) + 1) + "/" + list.length)
      resolve(1)
      return runtimeError_()
    })
  })
...
type DockDir = "left" | "right" | "up" | "down" | "center" | "max"
type WorkArea = { l: number, t: number, w: number, h: number }
type DockState = { dir: DockDir, level: number }

/** Per-window progressive dock level (cleared when direction changes). */
const dockStateByWnd_ = new Map<number, DockState>()

const normalizeDockDir_ = (raw: string): DockDir => {
  const d = (raw || "left").toLowerCase()
  if (d === "top") { return "up" }
  if (d === "bottom") { return "down" }
  if (d === "maximize" || d === "maximum") { return "max" }
...
const getWorkArea_ = (wnd: chrome.windows.Window, cb: (area: WorkArea) => void): void => {
  type DisplayInfo = {
    isEnabled?: boolean
    workArea?: { left: number, top: number, width: number, height: number }
    bounds?: { left: number, top: number, width: number, height: number }
  }
  const sys = (browser_ as any).system as { display?: { getInfo (cb: (infos: DisplayInfo[]) => void): void } } | undefined
  const displayApi = sys && sys.display
  if (displayApi && displayApi.getInfo) {
    try {
      displayApi.getInfo((infos: DisplayInfo[]): void => {
        ...
        cb({ l: a.left | 0, t: a.top | 0, w: a.width | 0, h: a.height | 0 })
      })
      return
    } catch { /* fall through */ }
  }
...
const fallbackWorkArea_ = (wnd: chrome.windows.Window): WorkArea => {
 ...
 */
export const dockWindow = (resolve: OnCmdResolved): void | kBgCmd.dockWindow => {
  const dir = normalizeDockDir_((get_cOptions<C.dockWindow>().direction || "left") + "")
  const rawStep = get_cOptions<C.dockWindow>().step
  const stepPct = Math.max(25, Math.min(90,
      (rawStep != null && +rawStep > 0 ? +rawStep : 0)
      || (+settingsCache_.dockWindowStep || 0) || 50))
  const step = stepPct / 100
  const rawFirst = get_cOptions<C.dockWindow>().first
  const first = Math.max(0.25, Math.min(0.75, rawFirst != null && +rawFirst > 0 ? +rawFirst : 0.5))
  Windows_.getCurrent((wnd): void => {
    ...
    const wndId = wnd.id
    getWorkArea_(wnd, (work): void => {
      if (work.w < 200 || work.h < 200) {
        showHUD("Invalid display size")
        resolve(0)
        return
      }
      if (dir === "max") {
        dockStateByWnd_.delete(wndId)
        Windows_.update(wndId, { state: "maximized" }, (): void => {
          showHUD("Maximized")
          resolve(1)
          return runtimeError_()
        })
      ...
      if (dir === "center") {
        ...
        Windows_.update(wndId, {
          ...
          showHUD("Centered " + Math.round(first * 100) + "%")
          resolve(1)
          return runtimeError_()
        })
        return
      ...
      Windows_.update(wndId, {
        state: "normal", left, top, width, height
      }, (): void => {
        const pct = Math.round(frac * 100)
        const hint = dir === "up" ? " · again = full screen" : ""
        showHUD("Dock " + dir + " · " + pct + "%" + hint)
        resolve(1)
        return runtimeError_()
      })
    })
...
export const showLastDownload = (resolve: OnCmdResolved): void | kBgCmd.showLastDownload => {
  const downloads = browser_.downloads
  if (!downloads || !downloads.search) {
    showHUD("Downloads API unavailable")
    resolve(0)
    return
  }
  const wantOpen = !!get_cOptions<C.showLastDownload>().open
  const wantShow = get_cOptions<C.showLastDownload>().show !== false
  downloads.search({ orderBy: ["-startTime"], limit: 20, state: "complete" }, (items): void => {
    if (runtimeError_() || !items || !items.length) {
      // fall back to any recent including in-progress
      downloads.search({ orderBy: ["-startTime"], limit: 5 }, (items2): void => {
...
const finishDownload_ = (item: chrome.downloads.DownloadItem, wantOpen: boolean, wantShow: boolean
    , resolve: OnCmdResolved): void => {
  const downloads = browser_.downloads!
  try {
    if (wantOpen && downloads.open) {
      downloads.open(item.id)
      showHUD("Opened: " + (item.filename.split(/[/\\]/).pop() || "download"))
    } else if (wantShow && downloads.show) {
      downloads.show(item.id)
      showHUD("Revealed in folder")
    } else {
      tabsCreate({ url: "chrome://downloads" })
      showHUD("Opened downloads page")
    }
    resolve(1)
  } catch {
    tabsCreate({ url: "chrome://downloads" })
...
export const toggleBookmark = (tabs: [Tab], resolve: OnCmdResolved): void | kBgCmd.toggleBookmark => {
  const bookmarks = browser_.bookmarks
  if (!bookmarks) {
    showHUD("Bookmarks API unavailable")
    resolve(0)
    return
  }
  const tab = tabs[0]
  ...
  void Q_(bookmarks.search, { url }).then((found): void => {
    const list = found || []
    if (list.length) {
      const id = list[0].id
      void Q_(bookmarks.remove, id).then((): void => {
    ...
    void Q_(bookmarks.create, { title, url }).then((): void => {
      showHUD("Bookmarked")
      resolve(1)
    }, (): void => {
      showHUD("Could not create bookmark")
      resolve(0)
    })
  }, (): void => { resolve(0) })
}
