...
import { getFindCSS_cr_, set_getFindCSS_cr_, browser_ } from "./browser"
...
export const setMediaState_ = (key: MediaNS.kName, matched: boolean
  ...
  if (key === MediaNS.kName.PrefersColorScheme && broadcast) {
    try {
      browser_.storage.local.set({ vpUiDark: matched ? 1 : 0 })
    } catch { /* empty */ }
  }
}
...
updateHooks_.autoDarkMode = updateHooks_.autoReduceMotion = (value: 0 | 1 | 2 | boolean
    ...
    if (keyName === "autoDarkMode") {
      try {
        if (value === 2) {
          browser_.storage.local.remove("vpUiDark")
        } else {
          browser_.storage.local.set({ vpUiDark: value > 0 ? 1 : 0 })
        }
      } catch { /* empty */ }
    }
