...
import {
  openSidePanelImmediate_, openSidePanelBestEffort_, explainSidePanelGesture_
} from "./side_panel"
...
set_reqH_([
  ...
  /** kFgReq.key: */ (request: FgReq[kFgReq.key], port: Port | null): void => {
    ...
    if (registryEntry) {
      ...
      if (registryEntry.alias_ === kBgCmd.openSidePanel && registryEntry.background_ && port) {
        const tabId = (port as Frames.Port).s.tabId_
        const ok = openSidePanelImmediate_(tabId >= 0 ? tabId : undefined
            , curWndId_ >= 0 ? curWndId_ : undefined)
        if (!ok) {
          void openSidePanelBestEffort_(null).then((opened): void => {
            if (!opened) { explainSidePanelGesture_() }
          })
        }
        return
      }
