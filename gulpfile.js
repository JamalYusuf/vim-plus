...
var Tasks = {
  ...
  _manifest: function(cb) {
    ...
    const manifest_v2 = mv3 ? {} : readJSON("./manifest.v2.json")
    var minVer = getBuildItem("MinCVer"), browser = getBuildItem("BTypes");
    minVer = minVer ? (minVer | 0) : 0;
      if (mv3 && browser === BrowserType.Firefox) {
        // Firefox MV3 packaging is unsupported in Vim+ (Chrome-only fork)
      }
  ...
  dist: [["build/ts"], ["static", "manifest", "min/others", function (done) {
    ...
    let clone = shortCommit && `>>> git clone <vim-plus-repo-url>`
