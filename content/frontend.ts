...
import { destroyPageEnhance } from "./page_enhance"
...
set_safeDestroy(((silent): void => {
    ...
    ui_box && adjustUI(2);
    try { destroyPageEnhance() } catch { /* empty */ }
    ...
    silent || recordLog("Vim+ on %o has been destroyed at %o.")()
...
if (OnFirefox && isAsContent) {
  ((): void => {
    ...
    type WindowWithGetter = Window & { __VimiumPlus_priv__: GetterWrapper }
    let randomKey = 0, recvTick = 0, sendTick = 0
    const name = BuildStr.CoreGetterFuncName as string as "__VimiumPlus_priv__"
    ...
    try {
      ...
      void (grabBackFocus || isHTML_() && wnd[name])
