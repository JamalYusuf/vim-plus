    ...
    OnDocLoaded_(function (): void {
      ...
      isTop || timeout_(function (): void {
        ...
        if (!needToRetryParentClickable) { /* empty */ }
          ...
          console.log(`Vim+: extend click: ${count ? "add " + count : "no"} local items sent to the parent's set.`)
...
set_hookOnWnd((function (action: HookAction): void {
  ...
  if (action !== HookAction.Suppress) {
    ...
    // bfcache restore: reconnect the extension port without console noise
    OnChrome && f("pageshow", onPageShowPort as (ev: Event) => void, t)
    if (!OnEdge) {
      f("toggle", onToggle, t)
    }
  }
