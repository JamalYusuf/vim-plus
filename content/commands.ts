...
set_contentCommands_([
  ...
  /* kFgCmd.showHelpDialog: */ ((options: CmdOptions[kFgCmd.showHelpDialog]): any => {
    ...
    optLink = querySelector_unsafe_("#HOpt", box) as WithOnAuxclick | null,
    ...
    // Wiki / Options / chrome://shortcuts must go through the background — page anchors cannot open them.
    const wireLaunch = (el: Element | null | undefined): void => {
      if (!el) { return }
      const a = el as HTMLElement & WithOnAuxclick & { href?: string }
      const open = (event: MouseEventToPrevent): void => {
        if (event.button > 1) { return }
        prevent_(event)
        const raw = a.getAttribute("data-vim-url") || a.getAttribute("href") || a.href || ""
        if (!raw) { return }
        post_({ H: kFgReq.focusOrLaunch, u: raw })
        hideHelp!(event)
      }
      a.onclick = a.onauxclick = open
    }
    if (optLink && !locHref().startsWith(optionUrl)) {
      wireLaunch(optLink)
      // Prefer options page URL from payload when present
      if (optionUrl) {
        optLink.setAttribute("data-vim-url", optionUrl)
      }
    } else if (optLink) {
      removeEl_s(optLink)
    }
    const launchers = querySelectorAll_unsafe_(".HLaunch", box) || []
    for (let i = 0; i < launchers.length; i++) {
      const node = launchers[i]
      if (optLink && node === optLink) { continue }
      wireLaunch(node)
    }
  ...
  /* kFgCmd.pictureInPicture: */ (): void => {
    type VideoEl = HTMLVideoElement & {
      requestPictureInPicture?: () => Promise<unknown>
      disablePictureInPicture?: boolean
      currentSrc?: string
    }
    const docPip = doc as Document & {
      exitPictureInPicture?: () => Promise<void>
      pictureInPictureElement?: Element | null
    }
    if (docPip.pictureInPictureElement && docPip.exitPictureInPicture) {
      void docPip.exitPictureInPicture().then((): void => { hudTip(kTip.raw, 0, "Exited PiP") }
          , (): void => { hudTip(kTip.raw, 0, "PiP exit failed") })
      return
    }
    const raw = querySelectorAll_unsafe_("video") as unknown as ArrayLike<VideoEl> | null | undefined
    const videos = raw || ([] as VideoEl[])
    let best: VideoEl | null = null, bestArea = 0
    for (let i = 0; i < videos.length; i++) {
      const v = videos[i]
      if (!v || v.disablePictureInPicture || !v.requestPictureInPicture) { continue }
      const r = getBoundingClientRect_(v)
      const area = Math.max(0, r.width) * Math.max(0, r.height)
      if (area > bestArea || (area === bestArea && !v.paused)) {
        best = v
        bestArea = area
      }
    }
    if (!best || !best.requestPictureInPicture) {
      hudTip(kTip.raw, 0, "No video for Picture-in-Picture")
      return
    }
    try {
      if (best.paused) {
        const p = best.play() as unknown as Promise<void> | void
        if (p && typeof (p as Promise<void>).then === "function") {
          void (p as Promise<void>).then((): void => { /* empty */ }, (): void => { /* empty */ })
        }
      }
    } catch { /* empty */ }
    void best.requestPictureInPicture().then((): void => { hudTip(kTip.raw, 0, "Picture-in-Picture") }
        , (): void => { hudTip(kTip.raw, 0, "PiP needs a user gesture on the video first") })
  }
])
