...
(function (): void {
  ...
  ref[0](kFgReq.i18n, 0, (res): void => {
    i18nMessages = res;
    if (VApi!.z) {
      VimPlusInjector!.$r(InjectorTask.extInited)
    }
  })
  ...
  const parentInjector = top !== window
      && safeFrameElement_()
      && (parent as Window & {VimPlusInjector?: typeof VimPlusInjector}).VimPlusInjector,
  ...
  if (runtime.onMessageExternal) {
    ...
    console.log("%cVim+%c: injected %cpartly%c into %c%s"
