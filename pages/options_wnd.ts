...
let optionsInit1_ = function (): void {
  ...
  if (OnChrome && Build.MinCVer < BrowserVer.MinEnsuredChromeURL$ExtensionShortcuts
    ...
    nextTick_(([el, el2, el3, el4]): void => {
      ...
      const el1 = el.parentElement as HTMLElement
      ...
      // Vim+: keep internal wiki docs instead of third-party companion extensions
      el2.href = "wiki.html#chrome-shortcuts"
      el3.href = "wiki.html#newtab-url"
  ...
  nextTick_((el): void => {
    if (el && !el.textContent) {
      el.textContent = "front/vomnibar.html"
    }
  }, $("#chromeExtVomnibar"));
...
delayBinding_("#resetDefaults", "click", (): void => {
  const ok = window.confirm(
      "Reset all Vim+ options to the author defaults?\n\n"
      + "This replaces your current settings with the recommended template.\n"
      + "Export a backup first if you want to keep your maps.")
  if (!ok) { return }
  const el2 = $<HTMLSelectElement>("#importOptions")
  if (!el2) { return }
  const oldSelected = el2.selectedIndex
  const rec = $<HTMLOptionElement>("#recommendedSettings")
  if (rec) { rec.selected = true }
  const finish = (): void => {
    if (el2.onchange) { el2.onchange(null as never) }
...
document.addEventListener("keydown", (event): void => {
  if ((Build.OS === kBOS.MAC as number || Build.OS & kBOS.MAC && !PageOs_) && event.metaKey) {
    ...
    if (key === "a-" + kChar.f12) {
      ...
      if (!Build.NDEBUG) {
        console.log('The document receives a "?" key which has been passed (excluded) by Vim+,',
