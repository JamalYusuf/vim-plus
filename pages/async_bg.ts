...
if (hasPageI18n) {
  if (!!Build.NDEBUG && !(Build.BTypes & BrowserType.Edge)
      && !(Build.BTypes & BrowserType.Firefox && Build.MinFFVer < FirefoxBrowserVer.MinEnsuredES$TopLevelAwait)
      && !(Build.BTypes & BrowserType.Chrome && Build.MinCVer < BrowserVer.MinEnsuredES$TopLevelAwait)) {
    onDicts_( // eslint-disable-next-line spaced-comment
      ...
      Promise.all(
        ...
        import(
          `/i18n/${lang}/${pageI18nName}.js`))).catch((): null[] => {
            console.log("Can not load page i18n modules for", pageI18nName)
            return []
          }) as never
    )
  } else {
    void Promise.all(pageLangs_.split(",").map((lang): Promise<string | null> => {
      const langFile = `/i18n/${lang}/${pageI18nName}.${Build.NDEBUG ? "js" : "json"}`
      const p: Promise<string> = (!OnChrome || Build.MinCVer >= BrowserVer.MinFetchExtensionFiles
          || CurCVer_ >= BrowserVer.MinFetchExtensionFiles
          ? (fetch as ValidFetch)(langFile).then((r): Promise<string> => {
        ...
        req.onload = function (): void {
          ...
          else { reject(new Error("XHR " + this.status)) }
        }
        req.onerror = (): void => { reject(new Error("XHR network")) }
...
((): void => {
  ...
  ? (branchedFunc: () => YieldableFunc, self: unknown): Promise<any> => new Promise<any> ((resolve): void => {
    ...
    function logDebugAndResolve(err: any): void {
      console.log("Vim+: an async function fails:", err)
      resolveVoid()
    }
  })
  : <TNext, TReturn> (generatorFunction: () => Generator<TNext | TReturn | Promise<TNext | TReturn>, TReturn, TNext>
    ...
    resume_()
    function logDebugAndResolve(err: any): void {
      if (!Build.NDEBUG) { console.log("Vim+: an async function fails:", err) }
      resolveVoid()
    }
  })
...
if (OnChrome ? (Build.MinCVer >= BrowserVer.MinMediaQuery$PrefersColorScheme
  ...
  const storage = browser_.storage.local as {
    get (k: string[] | Keys, cb: (r: { [k: string]: any }) => void): void
  }
  // Follow gn / vomnibar dark (vpUiDark) first, then Options autoDarkMode
  storage.get(["vpUiDark", "autoDarkMode"] as never, (res): void => {
  ...
  try {
    chrome.storage.onChanged.addListener((changes: { [k: string]: { newValue?: unknown } }, area: string): void => {
      ...
      if (changes["vpUiDark"]) {
        ...
        if (v === 1 || v === true || v === "1") { toggleDark_(1) }
        else if (v === 0 || v === false || v === "0") { toggleDark_(0) }
      } else if (changes["autoDarkMode"]) {
        const value = changes["autoDarkMode"].newValue as SettingsNS.PersistentSettings["autoDarkMode"] | boolean
        if (value === false || value === 0) { toggleDark_(0) }
        else if (value === 1 || value === true) { toggleDark_(1) }
        else { toggleDark_(2) }
      }
    })
  } catch { /* empty */ }
