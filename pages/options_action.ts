import {
  ...
  toggleReduceMotion_, hasShift_, PageOs_, setupPageOs_, prevent_, CurCVer_, escapeAllForRe_, browser_
} from "./async_bg"
import {
...
const initQuickActions = (): void => {
  ...
  const shortcuts = document.getElementById("qa-shortcuts") as HTMLAnchorElement | null
  if (helpBtn) {
    helpBtn.addEventListener("click", (): void => {
      void post_(kPgReq.initHelp)
    })
  }
  if (panelBtn) {
    panelBtn.addEventListener("click", (): void => {
      try {
        const sp = (browser_ as Dict<any>)["sidePanel"] as {
          open (opts: { tabId: number }): Promise<void>
        } | undefined
        if (sp && conf_.tabId >= 0) {
          void sp.open({ tabId: conf_.tabId })
        }
      } catch { /* empty */ }
    })
  }
  if (readingBtn) {
    readingBtn.addEventListener("click", (): void => {
      void post_(kPgReq.runPageAction, { action: "readingList", tabId: conf_.tabId }).then((res): void => {
        if (res && res.message) {
  ...
  if (bookmarkBtn) {
    bookmarkBtn.addEventListener("click", (): void => {
      void post_(kPgReq.runPageAction, { action: "bookmark", tabId: conf_.tabId }).then((res): void => {
        if (res && res.message) {
          const el = document.getElementById("state-action")
          if (el) { el.textContent = " · " + res.message }
        }
  ...
  if (shortcuts) {
    shortcuts.addEventListener("click", (event): void => {
      prevent_(event as unknown as EventToPrevent)
      void post_(kPgReq.focusOrLaunch, { u: "chrome://extensions/shortcuts" })
    })
  }
}
