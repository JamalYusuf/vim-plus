 ...
 */
  ...
  heightIfEmpty_: VomnibarNS.PixelData.OthersIfEmpty + 72,
  baseHeightIfNotEmpty_: VomnibarNS.PixelData.OthersIfNotEmpty + 72,
  ...
  tipEl_: null as HTMLElement | null,
  tipTextEl_: null as HTMLElement | null,
  tipModeEl_: null as HTMLElement | null,
  footerTipEl_: null as HTMLElement | null,
  styles_: "",
  customCss_: "",
  ...
  updateSelection_ (sel: number): void {
    ...
    if (sel >= 0 && ref[sel]) {
      try {
        (ref[sel] as HTMLElement).scrollIntoView({ block: "nearest" } as ScrollIntoViewOptions)
        Vomnibar_.updateListScrollHint_()
      } catch {
        try { (ref[sel] as HTMLElement).scrollIntoView(false) } catch { /* empty */ }
      }
    }
  },
  ...
  omni_ (response: BgVomnibarSpecialReq[kBgReq.omni_omni]): void {
    ...
    if (wrap) { (wrap as HTMLElement).style.maxHeight = listMax + "px" }
  ...
  bindListScrollHint_ (list: HTMLElement): void {
    const a = Vomnibar_
    if ((list as any).__vpScrollBound) { return }
    ;(list as any).__vpScrollBound = 1
    const onScroll = (): void => { a.updateListScrollHint_(list) }
    list.addEventListener("scroll", onScroll, { passive: true })
  },
  updateListScrollHint_ (list?: HTMLElement | null): void {
    ...
    if (canScroll && el.style.overflowY !== "auto") {
  ...
  updateOptions_ (delta: Req.bg<kBgReq.omni_updateOptions>["d"], confVer: number): void {
    ...
    if (delta.s != null) {
      ...
      Vomnibar_.heightIfEmpty_ = M(24, m((n || VomnibarNS.PixelData.OthersIfEmpty) + tipsChrome, 400));
      n = +sizes[1];
      Vomnibar_.baseHeightIfNotEmpty_ = M(24, m(Vomnibar_.heightIfEmpty_
          + (n || (VomnibarNS.PixelData.OthersIfNotEmpty - VomnibarNS.PixelData.OthersIfEmpty)), 400));
      n = +sizes[2];
      // Prefer CSS row height (48) if stored size is the old classic 44
      const rawItem = n || VomnibarNS.PixelData.Item
      Vomnibar_.itemHeight_ = M(14, m(rawItem === 44 ? VomnibarNS.PixelData.Item : rawItem, 120));
  ...
  updateTips_ (query: string, mode: string, count: number): void {
    ...
    if (q.startsWith(":")) {
      ...
      if (!rest) {
        html = "<b>Categories</b> · <kbd>:view</kbd> <kbd>:read</kbd> <kbd>:tab</kbd> <kbd>:hist</kbd> "
            + "<kbd>:priv</kbd> <kbd>:win</kbd> <kbd>:nav</kbd> <kbd>:clip</kbd> <kbd>:chrome</kbd>"
      } else if ((<RegExpOne> /^(view|read|tab|hist|priv|win|nav|clip|chrome|vim|fx|dock)$/).test(rest.split(/\s+/)[0] || "")) {
        html = "<b>Browsing " + (rest.split(/\s+/)[0] || "") + "</b> · filter further · <kbd>Enter</kbd> run · short form stays in bar"
      } else {
        html = "<b>Filter</b> · matches title, category &amp; cmd · hold <kbd>" + modKey
            + "</kbd> + <kbd>1</kbd>–<kbd>9</kbd> to jump"
      }
    } else if (q.startsWith("!")) {
      modeLabel = "ENGINE"
      html = "<b>Search engine</b> · <kbd>!g</kbd> Google · <kbd>!w</kbd> Wiki · <kbd>!gh</kbd> GitHub · query after bang"
    } else if (mode === "tab" || mode === "window" || modeLabel === "TAB" || modeLabel === "WINDOW") {
      modeLabel = mode === "window" || modeLabel === "WINDOW" ? "WINDOWS" : "TABS"
      html = "<b>" + modeLabel + "</b> · filter · <kbd>Enter</kbd> focus · hold <kbd>" + modKey
          + "</kbd> then <kbd>1</kbd>–<kbd>9</kbd> for numbered pick"
    } else if (!q) {
      modeLabel = "OMNI"
      html = "<b>Search everything</b> · <kbd>:</kbd> commands · <kbd>!g</kbd> engines · hold <kbd>"
          + modKey + "</kbd> for <kbd>1</kbd>–<kbd>9</kbd> hints"
    } else {
      modeLabel = "OMNI"
      html = "<b>" + (count ? count + " result" + (count === 1 ? "" : "s") : "No matches")
          + "</b> · hold <kbd>" + modKey + "</kbd> for numbered hints · <kbd>Enter</kbd> open"
    }
  ...
  init_ (): void {
    ...
    a.tipEl_ = document.getElementById("tips") as HTMLElement | null
...
(function (): void {
  ...
  onUnknownMsg = (event: MessageEvent): void => {
    ...
    if (!(data && data.length === 3 && data[0] === "VimiumPlus"
  ...
  Vomnibar_.secret_ = function (this: void, { l: payload, s: secret, v: confVersion }): void {
    ...
    if (!secret) { // see https://github.com/philc/vimium/issues/3832
      ...
      removeEventListener("message", onUnknownMsg, true)
      console.log("%cVim+: warning: Vomnibar was unexpectedly opened without triggering Vomnibar.activate!!!"
