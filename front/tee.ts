...
(function (): void {
  ...
  const onTask = (_response: BaseTeeTask): void => {
    ...
    const runTask = (): void | Promise<unknown> => {
      ...
      Build.NDEBUG || console.log("Vim+: error: unknown tee task id =", taskId)
    }
    const onFocus = (): void => {
      ...
      try {
        ...
        Build.NDEBUG || console.log("Vim+: error: failed in running task id = %o:\n%o", taskId, e)
      }
  ...
  port.onMessage.addListener((_response: unknown): void => {
    ...
    if (response.N !== kBgReq.omni_runTeeTask || refusedMoreMessages) {
      Build.NDEBUG || console.log("Vim+: error: unknown message:", response)
      destroy()
    } else {
